...
function test_presto_starts {
    ...
    if ! RESULT=$(docker exec "${CONTAINER_ID}" presto --execute "SELECT 'success'" 2>/dev/null); then
        echo "🚨 Failed to execute a query after Presto container started"
    fi
    set -e
...
function test_container {
    ...
    test_presto_starts "${CONTAINER_NAME}" "${PLATFORM}"
